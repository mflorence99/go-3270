...
export class Tn3270 {
  ...
  write(bytes: any): void {
    if (bytes instanceof Buffer) this.socket.write(bytes);
    else this.socket.write(Buffer.from(bytes));
  }

  // private methods

  private dataHandler(data: Buffer, observer: Observer<Buffer>): void {
    if (data[0] === Telnet.IAC) {
      ...
      if (response) {
        console.log(
          chalk.yellow('HOST -> 3270'),
          chalk.white(negotiator.decode())
        );
...
class Negotiator {
  ...
  private static reverse = Object.keys(Negotiator.lookup).reduce(
    (acc, k) => {
